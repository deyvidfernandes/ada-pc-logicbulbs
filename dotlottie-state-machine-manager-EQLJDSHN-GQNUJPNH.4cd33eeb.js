...
})({"87Mx3":[function(require,module,exports) {
var global = arguments[3];
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "b83376934cd33eeb";
"use strict";
...
},{}],"51ELB":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "DotLottieStateMachineManager", ()=>Nn) //# sourceMappingURL=out.js.map
;
...
function ne(e, t, n, i) {
    var r = e && n.reduce(function(o, a) {
        var s, c, h = a.assignment, f = {
            state: i,
            action: a,
            _event: t
        }, d = {};
        if (T(h)) d = h(o, t.data, f);
        else try {
...
function ve(e, t, n, i, r) {
    ...
    if (t.type === Rt) return (o?.[t.name] || t.predicate)(n, i.data, a);
    var s = o?.[t.type];
...
function jt(e, t, n, i, r, o, a) {
    a === void 0 && (a = !1);
    var s = a ? [] : Ye(r), c = s.length ? ne(n, i, s, t) : n, h = a ? [
        n
    ] : void 0, f = [];
    function d(y, p) {
        var b;
...
function Ge(e, t, n, i) {
    var r, o = pe(e.src), a = (r = t?.options.services) === null || r === void 0 ? void 0 : r[o.type], s = e.data ? Nt(e.data, n, i) : void 0, c = a ? ge(a, e.id, s) : ye(e.id);
    return c.meta = e, c;
}
function ge(e, t, n) {
    var i = ye(t);
...
function ht(e, t) {
    ...
    try {
        for(var l = E(u), y = l.next(); !y.done; y = l.next())for(var p = y.value, b = p.parent; b && !u.has(b);)u.add(b), b = b.parent;
    } catch (w) {
        n = {
            error: w
        };
    ...
    try {
        for(var m = E(u), _ = m.next(); !_.done; _ = m.next()){
            var p = _.value;
            if (p.type === "compound" && (!g.get(p) || !g.get(p).length)) d.get(p) ? d.get(p).forEach(function(M) {
                return u.add(M);
    ...
    try {
        for(var k = E(u), L = k.next(); !L.done; L = k.next())for(var p = L.value, b = p.parent; b && !u.has(b);)u.add(b), b = b.parent;
    } catch (w) {
        c = {
            error: w
        };
...
var F = function() {
    ...
    return e.from = function(t, n) {
        ...
        return !!i?.transitions.length && i.transitions.some(function(r) {
    ...
    function e(t) {
        this.processingEvent = !1, this.queue = [], this.initialized = !1, this.options = v(v({}, un), t);
    }
...
var pn = function() {
    function e(t, n) {
        ...
        var i = this;
        this.machine = t, this.delayedEventsMap = {}, this.listeners = new Set, this.contextListeners = new Set, this.stopListeners = new Set, this.doneListeners = new Set, this.eventListeners = new Set, this.sendListeners = new Set, this.initialized = !1, this.status = D.NotStarted, this.children = new Map, this.forwardTo = new Set, this._outgoingQueue = [], this.init = this.start, this.send = function(f, d) {
            if (ot(f)) return i.batch(f), i.state;
            var u = R(Lt(f, d));
            if (i.status === D.Stopped) return i.state;
            if (i.status !== D.Running && !i.options.deferEvents) throw new Error('Event "'.concat(u.name, '" was sent to uninitialized service "').concat(i.machine.id, `". Make sure .start() is called for this service, or set { deferEvents: true } in the service options.
Event: `).concat(JSON.stringify(u.data)));
            ...
            if (!y) {
                if (!l) throw new Error("Unable to send event to child '".concat(d, "' from service '").concat(i.id, "'."));
                return;
            }
            if ("machine" in y) {
                if (i.status !== D.Stopped || i.parent !== y || i.state.done) {
                    var p = v(v({}, f), {
                        name: f.name === Te ? "".concat(ut(i.id)) : f.name,
                        origin: i.sessionId
                    });
                    !u && i.machine.config.predictableActionArguments ? i._outgoingQueue.push([
                        y,
                        p
                    ]) : y.send(p);
                }
            } else !u && i.machine.config.predictableActionArguments ? i._outgoingQueue.push([
                y,
                f.data
            ]) : y.send(f.data);
            ...
            if (p) try {
                return p(d, u.data, i.machine.config.predictableActionArguments ? {
                    action: f,
                    _event: u
                } : {
                    action: f,
            ...
            switch(f.type){
                ...
                case Pt:
                    ...
                    if (typeof g.delay == "number") {
                        i.defer(g);
                        return;
                    } else g.to ? i.sendTo(g._event, g.to, u === nt) : i.send(g._event);
                    break;
                case oe:
                    i.cancel(f.sendId);
                    break;
        ...
        var r = v(v({}, e.defaultOptions), n), o = r.clock, a = r.logger, s = r.parent, c = r.id, h = c !== void 0 ? c : t.id;
        this.id = h, this.logger = a, this.clock = o, this.parent = s, this.options = r, this.scheduler = new re({
    ...
    return Object.defineProperty(e.prototype, "initialState", {
        ...
        else for(var u = void 0; u = this._outgoingQueue.shift();)u[0].send(u[1]);
        ...
        return this._stop(), n.schedule(function() {
            var i = R({
                ...
                var o = I(V([], O(t.state.configuration), !1).sort(function(f, d) {
                ...
                ], t.machine.config.predictableActionArguments ? t._exec : void 0, t.machine.config.predictableActionArguments || t.machine.config.preserveActionOrder), 2), s = a[0], c = a[1], h = new F({
                    value: t.state.value,
                    context: c,
                    _event: i,
        ...
        if (t.length) {
            ...
            this.scheduler.schedule(function() {
                var r, o, a = n.state, s = !1, c = [], h = function(l) {
                    var y = R(l);
        ...
        if (rt(t)) return this.spawnMachine(t, v(v({}, i), {
            id: n
        }));
        ...
        var i = this;
        n === void 0 && (n = {});
        var r = new e(t, v(v({}, this.options), {
            parent: this,
            id: n.id || t.id
        })), o = v(v({}, vn), n);
        ...
        var i, r = this, o = !1, a = new Set, s = new Set, c, h = function(u) {
        ...
        if (ee(f)) return this.spawnPromise(f, n);
        var d = (i = {
            id: n,
            send: function(u) {
...
var St = "", Ut = "#", It = "*", Y = {}, G = function(e) {
    ...
    function e(t, n, i, r) {
        ...
        }, this.idMap = {}, this.tags = [], this.options = Object.assign(gn(), n), this.parent = r?.parent, this.key = this.config.key || r?.key || this.config.id || "(machine)", this.machine = this.parent ? this.parent.machine : this, this.path = this.parent ? this.parent.path.concat(this.key) : [], this.delimiter = this.config.delimiter || (this.parent ? this.parent.delimiter : ue), this.id = this.config.id || V([
            this.machine.key
        ], O(this.path), !1).join(this.delimiter), this.version = this.parent ? this.parent.version : this.config.version, this.type = this.config.type || (this.config.parallel ? "parallel" : this.config.states && Object.keys(this.config.states).length ? "compound" : this.config.history ? "history" : "atomic"), this.schema = this.parent ? this.machine.schema : (a = this.config.schema) !== null && a !== void 0 ? a : {}, this.description = this.config.description, this.initial = this.config.initial, this.states = this.config.states ? ct(this.config.states, function(h, f) {
            var d, u = new e(h, {}, void 0, {
                parent: o,
                key: f
            });
            return Object.assign(o.idMap, v((d = {}, d[u.id] = u, d), u.idMap)), u;
        }) : Y;
        var s = 0;
        function c(h) {
            var f, d;
        ...
        c(this), this.history = this.config.history === !0 ? "shallow" : this.config.history || !1, this._transient = !!this.config.always || (this.config.on ? Array.isArray(this.config.on) ? this.config.on.some(function(h) {
            var f = h.event;
            return f === St;
        }) : St in this.config.on : !1), this.strict = !!this.config.strict, this.onEntry = U(this.config.entry || this.config.onEntry).map(function(h) {
            return dt(h);
            ...
            if (rt(h)) {
                var l = _t(o.id, f);
                return o.machine.options.services = v((d = {}, d[l] = h, d), o.machine.options.services), xt({
                    src: l,
                    id: l
                });
            } else if (A(h.src)) {
                ...
                var l = h.id || _t(o.id, f);
                return o.machine.options.services = v((u = {}, u[l] = h.src, u), o.machine.options.services), xt(v(v({
                    id: l
                }, h), {
                    src: l
                }));
    ...
    return e.prototype._init = function() {
        ...
        return new e(this.config, {
            actions: v(v({}, r), t.actions),
            activities: v(v({}, o), t.activities),
            guards: v(v({}, a), t.guards),
            services: v(v({}, s), t.services),
            delays: v(v({}, c), t.delays)
        }, n ?? this.context);
        ...
        return r.map(function(o) {
            var a = o.delay;
            return v(v({}, t.formatTransition(o)), {
                delay: a
            });
        });
        ...
        var n, i = this;
        if (!t) return [];
        var r = t instanceof F ? t.value : lt(t, this.delimiter);
        if (A(r)) {
        ...
        try {
            for(var f = E(this.getCandidates(a)), d = f.next(); !d.done; d = f.next()){
                var u = d.value, l = u.cond, y = u.in, p = t.context, b = y ? A(y) && G(y) ? t.matches(lt(this.getStateNodeById(y).path, this.delimiter)) : Xt(lt(y, this.delimiter), Le(this.path.slice(0, -2))(t.value)) : !0, g = !1;
                try {
...
`).concat(S.message));
                ...
                if (g && b) {
                    u.target !== void 0 && (c = u.target), s.push.apply(s, V([], O(u.actions), !1)), h = u;
                    break;
                }
            }
        } catch (S) {
        ...
        var c, h, f, d, u = this, l = a ? ht([], this.getStateNodes(a.value)) : [], y = new Set;
        try {
        ...
        var S = Array.from(y).sort(function(x, j) {
            ...
            return {
                type: "entry",
                actions: Q(s ? V(V([], O(j), !1), O(z), !1) : V(V([], O(z), !1), O(j), !1), u.machine.options.actions)
            };
        }).concat({
            type: "state_done",
        ...
        var h = this._transition(a.value, a, o) || {
            transitions: [],
            configuration: [],
            exitSet: [],
            source: a,
            actions: []
        }, f = ht([], this.getStateNodes(a.value)), d = h.configuration.length ? ht(f, h.configuration) : f;
        return h.configuration = V([], O(d), !1), this.resolveTransition(h, a, a.context, r, o);
        ...
        var a, s, c, h, f = this;
        o === void 0 && (o = nt);
        var d = t.configuration, u = !n || t.transitions.length > 0, l = u ? t.configuration : n ? n.configuration : [], y = kt(l, this), p = u ? rn(this.machine, d) : void 0, b = n ? n.historyValue ? n.historyValue : t.source ? this.machine.historyValue(n.value) : void 0 : void 0, g = this.getActions(new Set(l), y, t, i, o, n, r), m = n ? v({}, n.activities) : {};
        try {
        ...
        }, n ? v({}, n.children) : {}), $ = new F({
            value: p || n.value,
            context: x,
            _event: o,
            _sessionid: n ? n._sessionid : null,
            historyValue: p ? b ? De(b, p) : void 0 : n ? n.historyValue : void 0,
            history: !p || t.source ? n : void 0,
            actions: p ? B : [],
            activities: p ? m : n ? n.activities : {},
            events: [],
            configuration: l,
...
`).concat(s.message));
        ...
        }) : !0, o = this.machine.options.guards, a = this.resolveTarget(i), s = v(v({}, t), {
        ...
        if (!this.config.on) r = [];
            ...
            var o = this.config.on, a = It, s = o[a], c = s === void 0 ? [] : s, h = $t(o, [
                typeof a == "symbol" ? a : a + ""
            ]);
        ...
        var f = this.config.always ? Z("", this.config.always) : [], d = this.config.onDone ? Z(String(bt(this.id)), this.config.onDone) : [], u = I(this.invoke.map(function(m) {
            var _ = [];
            return m.onDone && _.push.apply(_, V([], O(Z(String(Ot(m.id)), m.onDone)), !1)), m.onError && _.push.apply(_, V([], O(Z(String(ut(m.id)), m.onError)), !1)), _;
        })), l = this.after, y = I(V(V(V(V([], O(d), !1), O(u), !1), O(r), !1), O(f), !1).map(function(m) {
...
Object.values(et);
var bn = Object.keys(et), Nn = class {
    activeStateId = "";
    _service;
    ...
    start(e) {
        ...
        if (typeof t > "u") throw (0, _chunkZDBUHXQ5Mjs.a)(`invalid state machine id ${e}`);
        this._service = Se(_n(t)), this._addEventListeners(), this._service.start();
    }
    stop() {
        this._removeEventListeners(), this._service?.stop(), this._player.stop();
    }
    _removeEventListeners() {
        this._requiresDomElement();
        for (let [e, t] of this._domListeners)e === "visibilityChange" && this._player.stopPlayOnShow(), this._domElement?.removeEventListener(e, t), this._domListeners.delete(e);
        for (let [e, t] of this._playerListeners)this._player.removeEventListener(e, t), this._playerListeners.delete(e);
    }
    _addEventListeners() {
        this._requiresDomElement();
        let e = (n)=>{
        ...
        this._service.subscribe((n)=>{
            if (typeof n.changed > "u" || n.changed) {
                ...
                for (let i of n.nextEvents)if (bn.filter((r)=>r !== "complete" && r !== "show").includes(i)) {
                    let r = t(i);
                    this._domListeners.set(i, r), this._domElement?.addEventListener(i, r, {
                        once: !0
                    });
                } else if (i === "complete") {
                    let r = t(i);
                    ...
                    this._player.addIntersectionObserver({
                        callbackOnIntersect: (o)=>{
                            o > 0 && (o !== 100 && this._onShowPrevValue !== o && r(), this._onShowPrevValue = o);
                        },
                        threshold: []
                    }), this._domListeners.set("visibilityChange", r);
                }
            }
        });
    }
    subscribe(e) {
        throw (0, _chunkZDBUHXQ5Mjs.a)(e.toString());
    }
            ...
            if (r.id = n.descriptor.id, typeof n.descriptor.initial < "u" && (r.initial = n.descriptor.initial), typeof n < "u") {
                ...
                for(let a in o)if (typeof o[a] < "u" && o[a]) {
                    ...
                    for (let u of h$1)if (typeof s[u] < "u") {
                        ...
                        } else u === et.enter ? f[(0, _chunkZDBUHXQ5Mjs.d)(et, u)] = {
                            target: l?.state ?? ""
                        } : f[(0, _chunkZDBUHXQ5Mjs.d)(et, u)] = {
                            target: l?.state ?? ""
                        };
                    }
                    i[a] = {
                        entry: ()=>{
                            !this._player.getAnimationInstance() || s.animationId && s.animationId !== this._player.currentAnimationId ? this._player.play(s.animationId, ()=>({
                                    ...(0, _chunkZDBUHXQ5Mjs.h),
                                    ...c
                                })).then(()=>{
                                this._updatePlaybackSettings(c);
                            }).catch((u)=>{
                                (0, _chunkZDBUHXQ5Mjs.b)(`State machine error: ${u.message}`);
                            }) : this._updatePlaybackSettings(c);
                        },
                        exit: ()=>{
    ...
    _requiresDomElement() {
        if (!this._domElement) throw (0, _chunkZDBUHXQ5Mjs.a)("Requires a DOM element to attach events.");
    }
...
},{"./chunk-ZDBUHXQ5.mjs":"hu351","./chunk-U2GK2GFR.mjs":"bBhes","./chunk-XNFEUXDS.mjs":"4Lvnj","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["87Mx3"], null, "parcelRequire8861")

//# sourceMappingURL=dotlottie-state-machine-manager-EQLJDSHN-GQNUJPNH.4cd33eeb.js.map
